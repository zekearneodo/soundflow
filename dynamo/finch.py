...
def finch(pars, int_par_stream, x_0=None):
    ...
    for step in np.arange(pars['max_tau'], pars['max_tau'] + total_steps):
        ...
        pars['A_1'] = np.sqrt(pars['sys']['envelope']) * (1. + pars['sys']['noise_fraction_env'] * pars['sys']['noise'][0])
        pars['sys']['beta_1'] = beta_1 * (1. + pars['sys']['noise_fraction_beta_1'] * pars['sys']['noise'][1])
